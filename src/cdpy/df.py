...
class CdpyDf(CdpSdkBase):
    ...
    def list_services(self, only_enabled=False, env_crn=None, df_crn=None, name=None):
        ...
        if only_enabled:
            result = [
                x for x in result if x["status"]["state"] in self.sdk.STARTED_STATES
            ]
        if name is not None:
        ...
        if df_crn is not None:
            ...
            if len(services) == 0:
                ...
                self.sdk.throw_error(
                    CdpError(
                        "More than one DataFlow service found for env_crn, please try list instead"
                    )
                )
        else:
            self.sdk.throw_error(
                CdpError(
                    "Either df_crn or env_crn must be supplied to df.describe_service"
                )
            )
        if resolved_df_crn is not None:
            return self.sdk.call(
                ...
                squelch=[
                    ...
                    ),  # DF GRPC sometimes returns 403 when finishing deletion
                ],
    ...
    def resolve_service_crn_from_name(self, name, only_enabled=True):
        ...
        if len(listing) == 1:
            ...
            self.sdk.throw_warning(
                CdpWarning("No DataFlow Service found matching name %s" % name)
            )
            return None
        else:
            self.sdk.throw_error(
                CdpError("Multiple DataFlow Services found matching name %s" % name)
            )
    ...
    def enable_service(
        ...
        return self.sdk.call(
            ...
            minK8sNodeCount=min_nodes,
            maxK8sNodeCount=max_nodes,
            usePublicLoadBalancer=enable_public_ip,
            privateCluster=private_cluster,
            kubeApiAuthorizedIpRanges=kube_ips,
            loadBalancerAuthorizedIpRanges=lb_ips,
    ...
    def disable_service(self, df_crn: str, persist: bool = False, terminate=False):
        ...
        return self.sdk.call(
            ...
            serviceCrn=df_crn,
            persist=persist,
            terminateDeployments=terminate,
        )

    def reset_service(self, df_crn: str):
        self.sdk.validate_crn(df_crn)
        return self.sdk.call(svc="df", func="reset_service", serviceCrn=df_crn)

    def list_deployments(
        self, env_crn=None, df_crn=None, name=None, dep_crn=None, described=False
    ):
        result = self.sdk.call(
            svc="df",
            func="list_deployments",
            ret_field="deployments",
            squelch=[
                Squelch(
                    value="NOT_FOUND",
                    default=list(),
    ...
    def describe_deployment(self, dep_crn=None, df_crn=None, name=None):
        if dep_crn is not None:
            ...
            if len(deployments) == 0:
                ...
                self.sdk.throw_error(
                    CdpError(
                        "More than one DataFlow Deployment found, please try list instead"
                    )
                )
        else:
            self.sdk.throw_error(
                CdpError("Either dep_crn or both of df_crn and name must be supplied")
            )
        return self.sdk.call(
            svc="df",
            func="describe_deployment",
            ret_field="deployment",
            squelch=[
                Squelch(
                    value="NOT_FOUND",
    ...
    def list_readyflows(self, name=None):
        ...
        result = self.sdk.call(
            svc="df",
            func="list_readyflows",
            ret_field="readyflows",
            squelch=[
                Squelch(
                    value="NOT_FOUND",
    ...
    def describe_readyflow(self, def_crn):
        ...
        return self.sdk.call(
            svc="df",
            func="describe_readyflow",
            ret_field="readyflowDetail",
            squelch=[
                Squelch(
                    value="NOT_FOUND",
    ...
    def import_readyflow(self, def_crn):
        ...
        return self.sdk.call(
            svc="df",
            func="add_readyflow",
            ret_field="addedReadyflowDetail",
            squelch=[
                Squelch(
                    value="NOT_FOUND",
                    warning="No ReadyFlow Definition with crn %s found" % def_crn,
    ...
    def describe_added_readyflow(self, def_crn, sort_versions=True):
        ...
        if sort_versions and out:
            out["versions"] = sorted(
                result["versions"], key=lambda d: d["version"], reverse=True
            )
        return out
    ...
    def import_customflow_version(self, def_crn, def_file, comments=None):
        ...
        return self.sdk.call(
            svc="df",
            func="import_flow_definition_version",
            ret_field=".",
            squelch=[
                Squelch(value="PATH_DISABLED", warning=ENTITLEMENT_DISABLED),
                Squelch(field="status_code", value="409"),
        ...
        if summary_list:
            if len(summary_list) == 1:
                ...
                if kind == "flow":
                    ...
                    detail = None
                    self.sdk.throw_error(
                        CdpError("DataFlow Definition type not supported %s" % kind)
                    )
                if version is None:
                    ...
                    if out:
                        ...
                        self.sdk.throw_error(
                            CdpError(
                                "Could not find version %d for DataFlow Definition named %s"
                                % (version, flow_name)
                            )
                        )
            else:
                self.sdk.throw_error(
                    CdpError(
                        "More than one DataFlow Definition found for name %s"
                        % flow_name
                    )
                )
        else:
            self.sdk.throw_warning(
                CdpWarning("DataFlow Definition not found for name %s" % flow_name)
            )
    ...
    def create_deployment(self, df_crn, flow_ver_crn, deployment_name, size_name=None, static_node_count=None,
        ...
        if size_name is not None and size_name not in self.DEPLOYMENT_SIZES:
            self.sdk.throw_error(
                CdpError(
                    "Deployment size_name %s not in supported size list: %s"
        ...
        if self.list_deployments(name=deployment_name):
            self.sdk.throw_error(
                CdpError(
                    "Deployment already exists with conflicting name %s"
                    % deployment_name
                )
            )
        # Setup
        ...
        if autoscale_enabled:
            ...
            config["autoScaleMinNodes"] = (
                autoscale_nodes_min if autoscale_nodes_min is not None else 1
            )
            config["autoScaleMaxNodes"] = (
                autoscale_nodes_max if autoscale_nodes_max is not None else 3
            )
        else:
            config["staticNodeCount"] = (
                static_node_count if static_node_count is not None else 1
            )

        # cdpcli/extensions/df/createdeployment.py  cdpcli-beta v0.9.48+
        dep_req_crn = self.sdk.call(
            svc="df",
            func="initiate_deployment",
            ret_field="deploymentRequestCrn",
            serviceCrn=df_crn,
            flowVersionCrn=flow_ver_crn,
        )
        df_handler = CreateDeploymentOperationCaller()
        df_handler._upload_assets(
            df_workload_client=self.sdk._client(
                service="dfworkload", parameters=config
            ),
    ...
    def terminate_deployment(self, dep_crn, env_crn=None):
        ...
        _ = [
            self.sdk.validate_crn(x[0], x[1])
            for x in [(env_crn, "env"), (dep_crn, "deployment")]
        ]
        return self.sdk.call(
            svc="dfworkload",
            func="terminate_deployment",
            ret_field="deployment",
            environmentCrn=env_crn,
            deploymentCrn=dep_crn,
        )
