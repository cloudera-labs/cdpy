...
class CdpyEnvironments(CdpSdkBase):
    ...
    def get_credential_prerequisites(self, cloud: str):
        return self.sdk.call(
            svc="environments",
            func="get_credential_prerequisites",
            cloudPlatform=cloud.upper(),
        )
    ...
    def create_proxy_config(
        ...
        return self.sdk.call(
            svc="environments",
            func="create_proxy_config",
            ret_field="proxyConfig",
    ...
    def delete_proxy_config(self, name):
        return self.sdk.call(
            svc="environments",
            func="delete_proxy_config",
            ret_field="credentials",
    ...
    def list_proxy_configs(self, name=None):
        return self.sdk.call(
            svc="environments",
            func="list_proxy_configs",
            ret_field="proxyConfigs",
            squelch=[Squelch("NOT_FOUND", default=list())],
            proxyConfigName=name,
        )
    ...
    def get_id_broker_mapping_sync(self, name):
        return self.sdk.call(
            svc="environments",
            func="get_id_broker_mappings_sync_status",
            squelch=[Squelch("NOT_FOUND"), Squelch("INVALID_ARGUMENT")],
    ...
    def describe_environment(self, name):
        resp = self.sdk.call(
            svc="environments",
            func="describe_environment",
            ret_field="environment",
            ret_error=True,
            environmentName=name,
        )
    ...
    def describe_all_environments(self):
        ...
        if envs_listing is not None:
            return [
                self.describe_environment(env["environmentName"])
                for env in envs_listing
            ]
        else:
            return list()
    ...
    def list_environments(self):
        return self.sdk.call(
            ...
            ret_field="environments",
            squelch=[
                Squelch(
                    "NOT_FOUND",
                    default=list(),
                    warning="No Environments found in CDP Tenant",
                )
            ],
        )
        ...
        if isinstance(resp, CdpError):
            if resp.error_code == "INVALID_ARGUMENT":
                if "constraintViolations" not in str(resp.violations):
                    resp.update(
                        message="Received violation warning:\n%s"
                        % self.sdk.dumps(str(resp.violations))
                    )
                    self.sdk.throw_warning(CdpWarning(str(resp.violations)))
            self.sdk.throw_error(resp)
        return resp
    ...
    def stop_environment(self, name):
        return self.sdk.call(
            svc="environments",
            func="stop_environment",
            ret_field="environment",
            squelch=[
                Squelch(
                    field="error_code",
                    value="CONFLICT",
                    default=self.describe_environment(name),
    ...
    def sync_users(self, environments=None):
        if isinstance(environments, list):
            ...
            self.sdk.throw_error(
                CdpError(
                    "environments must be a list of one or more strings or a string"
                )
            )
        resp = self.sdk.call(
            svc="environments",
        ...
        if isinstance(resp, CdpError):
            if resp.error_code == "CONFLICT":
                operation_match = self.sdk.regex_search(
                    self.sdk.OPERATION_REGEX, str(resp.violations)
                )
                if operation_match is not None:
                    ...
                    if not self.sdk.strict_errors:
                        self.sdk.throw_warning(
                            CdpWarning(
                                "Sync All Users Operation already running, tracking existing job {0}".format(
                                    existing_op_id
                                )
                            )
                        )
        ...
        if isinstance(resp, CdpError):
            if retries > 0:
                ...
                if any(x in str(resp.violations) for x in consistency_violations):
                    ...
                    self.sdk.throw_warning(
                        CdpWarning(
                            "Got likely AWS IAM eventual consistency error [%s], %d retries left..."
                            % (str(resp.violations), retries)
                        )
                    )
                    self.sdk.sleep(delay)
                    return self.create_aws_credential(
                        name, role, description, retries, delay
                    )
            else:
                self.sdk.throw_error(resp)
        return resp
    ...
    def create_azure_credential(
        self, name, subscription, tenant, application, secret, retries=3, delay=5
    ):
        resp = self.sdk.call(
            svc="environments",
        ...
        if isinstance(resp, CdpError):
            if retries > 0:
                ...
                if any(x in str(resp.violations) for x in consistency_violations):
                    ...
                    self.sdk.throw_warning(
                        CdpWarning(
                            "Got likely Azure eventual consistency error [%s], %d retries left..."
                            % (str(resp.violations), retries)
                        )
                    )
                    self.sdk.sleep(delay)
                    return self.create_azure_credential(
                        name, subscription, tenant, application, secret, retries, delay
                    )
            else:
                self.sdk.throw_error(resp)
        return resp
    ...
    def upgrade_freeipa(self, env, allow_major_os_upgrade=None, image_id=None):
        return self.sdk.call(
            svc="environments",
            func="upgrade_freeipa",
            squelch=[],
            environmentName=env,
            allowMajorOsUpgrade=allow_major_os_upgrade,
            image_id=image_id,
        )
    ...
    def get_freeipa_upgrade_options(
        self, env, catalog=None, allow_major_os_upgrade=None
    ):
        return self.sdk.call(
            svc="environments",
            func="get_freeipa_upgrade_options",
            squelch=[],
            environment=env,
            allowMajorOsUpgrade=allow_major_os_upgrade,
            catalog=catalog,
